...
const FlightCard = ({
  ...
  duration, price, priceEconomy, priceBusiness, stops, stopsDetail,
  logoUrl, selected, onSelect,
}: FlightCardProps) => {
  ...
  const businessPrice = priceBusiness && priceBusiness !== "Sob consulta" ? priceBusiness : null;
  const displayPrice = selectedClass === "economy" ? economyPrice : (businessPrice || "Sob consulta");
  ...
  return (
    <>
      ...
      >
        {/* Linha 1: logo + nome + badge paradas */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            {logoUrl ? (
              <img src={logoUrl} alt={airline} className="h-6 w-6 object-contain rounded-sm"
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="relative z-10 w-full max-w-md bg-card border border-border rounded-xl p-6 shadow-2xl"
            ...
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                {logoUrl ? (
                  <img src={logoUrl} alt={airline} className="h-7 object-contain"
                    onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                ) : (
            ...
            {/* Rota visual */}
            <div className="bg-muted/30 rounded-lg p-4 mb-4 flex items-center gap-4">
              <div className="text-center">
            ...
            {!isDirect && stopsDetail && (
              <div className="mb-4 p-3 rounded-lg border border-amber-800/40 bg-amber-950/20">
                <p className="text-xs text-amber-400 font-medium mb-1 uppercase tracking-wider">Detalhes da conexão</p>
            ...
            {/* Seletor de classe */}
            <div className="mb-5">
              <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">Selecione a classe</p>
              <div className="grid grid-cols-2 gap-2">
                ...
                <button onClick={() => setSelectedClass("business")}
                  ...
                  <p className={`text-base font-display font-bold ${
                    businessPrice ? "text-primary" : "text-muted-foreground text-sm"
                  }`}>{businessPrice || "Sob consulta"}</p>
                </button>
              </div>
            </div>
